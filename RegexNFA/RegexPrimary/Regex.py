...
if __name__ == '__main__':
    pattern = Regex('d')
    assert pattern.search('dog') is True
    assert pattern.search('dog', 1) is None

    pattern = Regex('o')
    assert pattern.match('dog') is None
    assert pattern.match('dog', 1) is True

    pattern = Regex('o[gh]')
    assert pattern.fullmatch('dog') is None
    assert pattern.fullmatch('ogre') is None
    assert pattern.fullmatch('doggie', 1, 3) is True
